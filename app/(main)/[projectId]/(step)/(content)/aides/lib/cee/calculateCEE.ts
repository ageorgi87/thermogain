...
export const calculateCEE = (input: CEEInput): CEEResult => {
  ...
  if (!input.logementPlusde2ans) {
    return {
      ...
      montant: 0,
      message:
        "❌ Non éligible : le logement doit avoir au moins 2 ans d'ancienneté.",
      details: [
        "Les CEE sont réservés aux logements existants de plus de 2 ans.",
      ],
    };
  }
  ...
  if (montant === 0) {
    ...
    if (input.typePac === "Air/Air" && category === "classique") {
      return {
        ...
        montant: 0,
        message:
          "❌ Non éligible : les PAC Air/Air ne bénéficient du CEE Coup de Pouce que pour les ménages précaires ou modestes.",
        details: [
          ...details,
