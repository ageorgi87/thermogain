import { ELECTRIC_EFFICIENCY } from "../config/heatingEfficiencyData";
...
export const calculateBoilerEfficiency = (
  ...
  if (!["Bon", "Moyen", "Mauvais"].includes(condition)) {
    console.warn(
      `État d'entretien invalide: ${condition}, utilisation de "Moyen" par défaut`
    );
    condition = "Moyen";
  }
  ...
  switch (heatingType) {
    case "Gaz":
      ...
      const gasIsCondensing =
        isCondensing !== undefined ? isCondensing : age < 15;
    ...
    default:
      console.warn(
        `Type de chauffage inconnu: ${heatingType}, utilisation de 0.75 par défaut`
      );
      return 0.75;
  }
};
