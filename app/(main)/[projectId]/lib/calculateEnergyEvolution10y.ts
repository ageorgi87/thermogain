 ...
 */
export const calculateEnergyEvolution10y = async (
  rid: string,
  ...
  try {
    ...
    if (rows.length < MIN_REQUIRED_MONTHS) {
      console.warn(
        `Pas assez de données pour calculer l'évolution pour ${priceColumnName} (${rows.length}/${MIN_REQUIRED_MONTHS})`
      );
      return 0;
    }
    ...
    if (prices.length < MIN_REQUIRED_MONTHS) {
      console.warn(
        `Pas assez de prix valides pour ${priceColumnName} (${prices.length}/${MIN_REQUIRED_MONTHS} trouvés)`
      );
      return 0;
    }
    ...
    console.log(
      `📊 Calcul de l'évolution pondérée pour ${priceColumnName} sur ${prices.length} mois (${yearsOfData.toFixed(1)} ans)`
    );
    ...
    console.log(
      `   📈 Évolution long terme (${yearsOfData.toFixed(1)} ans): ${evolutionLongTerm.toFixed(2)}%/an`
    );
    ...
    if (prices.length >= 120) {
      ...
      console.log(
        `   ℹ️  Pas assez de données pour calcul 10 ans (${prices.length}/120), utilisation évolution long terme`
      );
    }
    ...
    console.log(
      `   🎯 Évolution pondérée finale (30% LT + 70% 10y): ${weightedEvolution.toFixed(2)}%/an`
    );
    console.log(
      `   Prix ancien: ${oldestAvg.toFixed(2)} €/100kWh → Prix récent: ${recentAvg.toFixed(2)} €/100kWh`
    );
    ...
    console.error(
      `Erreur lors du calcul de l'évolution pour ${priceColumnName}:`,
      error
    );
    return 0;
  }
