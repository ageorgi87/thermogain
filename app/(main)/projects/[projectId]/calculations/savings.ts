 ...
 */
export function calculateYearlyData(data: ProjectData, years: number): YearlyData[] {
  const yearlyData: YearlyData[] = []
  let economiesCumulees = 0
  const currentYear = new Date().getFullYear()

  for (let i = 0; i < years; i++) {
    const coutActuel = calculateCurrentCostForYear(data, i)
    const coutPac = calculatePacCostForYear(data, i)
 ...
 */
export function calculateTotalSavings(data: ProjectData, years: number): number {
  const yearlyData = calculateYearlyData(data, years)
 ...
 */
export function calculateNetBenefit(data: ProjectData, years: number): number {
  const yearlyData = calculateYearlyData(data, years)
  ...
  const coutTotalPac = data.reste_a_charge + yearlyData.reduce((sum, y) => sum + y.coutPac, 0)
 ...
 */
export function calculateGainsAfterROI(data: ProjectData, years: number): number {
  const yearlyData = calculateYearlyData(data, years)
