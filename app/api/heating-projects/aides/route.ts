...
import { aidesSchema } from "@/lib/schemas/heating-form"
...
export async function POST(req: Request) {
  try {
    ...
    const { heatingProjectId, ...aidesData } = body
    const validatedData = aidesSchema.parse(aidesData)
    ...
    // Upsert aides data
    const aides = await prisma.heatingProjectAides.upsert({
    ...
    if (heatingProject.currentStep === 6) {
    ...
    return NextResponse.json(aides)
  } catch (error) {
    console.error("Error saving aides data:", error)
